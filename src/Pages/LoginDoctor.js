...
const LoginDoctor = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (json.success) {
      ...
      navigate("/StartDoctor"); // Navigate to the /startdoctor route
    } else {
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="w-50">
        <h5 align="Center" fontsize="24px" >Doctor Login</h5>
        <div className="mb-3">
        ...
        <p className="mt-3">
          Don't have an account? <Link to="/SignupDoctor">Create one</Link>
        </p>
      </form>
    </div>
  );
